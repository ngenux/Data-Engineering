...
-- MAGIC %python
-- MAGIC assert spark.table("beans_1"), "Table named `beans` does not exist"
-- MAGIC assert spark.table("beans_1").columns == ["name", "color", "grams", "delicious"], "Please name the columns in the order provided above"
-- MAGIC assert spark.table("beans_1").dtypes == [("name", "string"), ("color", "string"), ("grams", "float"), ("delicious", "boolean")], "Please make sure the column types are identical to those provided above"
...
-- MAGIC %python
-- MAGIC assert spark.table("beans_1").count() == 6, "The table should have 6 records"
...
-- MAGIC assert set(row["name"] for row in spark.table("beans_1").select("name").collect()) == {'beanbag chair', 'black', 'green', 'jelly', 'lentils', 'pinto'}, "Make sure you have not modified the data provided"
...
-- MAGIC %python
-- MAGIC assert spark.table("beans_1").filter("name='pinto'").count() == 1, "There should only be 1 entry for pinto beans"
-- MAGIC row = spark.table("beans_1").filter("name='pinto'").first()
...
-- MAGIC %python
-- MAGIC assert spark.table("beans_1").filter("delicious=true").count() == 5, "There should be 5 delicious beans in your table"
-- MAGIC assert spark.table("beans_1").filter("name='beanbag chair'").count() == 0, "Make sure your logic deletes non-delicious beans"
...
-- MAGIC %python
-- MAGIC version = spark.sql("DESCRIBE HISTORY beans_1").selectExpr("max(version)").first()[0]
-- MAGIC last_tx = spark.sql("DESCRIBE HISTORY beans_1").filter(f"version={version}")
...
-- MAGIC %python
-- MAGIC assert spark.sql("SHOW TABLES LIKE 'beans_1'").collect() == [], "Confirm that you have dropped the `beans` table from your current database"
