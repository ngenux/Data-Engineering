...
SELECT * FROM beans_1 VERSION AS OF 1
...
select * from beans_1 version as of 4
...
-- TODO
restore table beans_1 version as of 5
...
-- MAGIC assert spark.sql(f"DESCRIBE HISTORY beans_1").select("operation").first()[0] == "RESTORE", "Make sure you reverted your table with the `RESTORE` keyword"
-- MAGIC assert spark.table("beans_1").count() == 5, "Make sure you reverted to the version after deleting records but before merging"
...
-- MAGIC %python
-- MAGIC last_tx = spark.sql("DESCRIBE HISTORY beans_1").first()
...
VACUUM beans_1 RETAIN 0 HOURS DRY RUN
